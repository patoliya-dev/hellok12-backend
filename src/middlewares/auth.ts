...
export const requireAllRoles = (requiredRoles: string[]) => {
  return (req: AuthenticatedRequest, res: Response, next: NextFunction): void => {
    ...
    // For now, users have single roles, but this can be extended for multi-role systems
    const hasAllRoles = requiredRoles.every(role => req.user?.role === role);
...
// School-specific authorization (for school admins managing their teachers)
export const requireSchoolAccess = (
  ...
  if (req.user.role !== USER_ROLES.SCHOOL && req.user.role !== USER_ROLES.SUPER_ADMIN) {
    res
      .status(403)
      .json(createErrorResponse('Access denied. School admin access required', 'Forbidden', 403));
    return;
  }
...
// Teacher access validation (teacher can only access their own resources)
export const requireTeacherAccess = (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
): void => {
  ...
  if (req.user.role === USER_ROLES.TEACHER && req.user.id !== teacherId) {
    res
      .status(403)
      .json(
        createErrorResponse(
          'Access denied. Teachers can only access their own resources',
          'Forbidden',
          403
        )
      );
  ...
  if (
    ...
    res
      ...
      .json(
        createErrorResponse(
          'Access denied. Teacher, school, or admin access required',
          'Forbidden',
          403
        )
      );
...
// Parent access validation (parent can only access their own children's resources)
export const requireParentAccess = (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
): void => {
  ...
  if (req.user.role !== USER_ROLES.PARENT && req.user.role !== USER_ROLES.SUPER_ADMIN) {
    res
      .status(403)
      .json(
        createErrorResponse('Access denied. Parent or admin access required', 'Forbidden', 403)
      );
    return;
  }
...
export const authenticateWithCookie = async (
  ...
  try {
    ...
    if (!token) {
      res
        .status(401)
        .json(createErrorResponse('No authentication cookie found', 'Unauthorized', 401));
      return;
    }
...
export const validateToken = (token: string): UserPayload | null => {
  try {
    const secret = JWT_SECRET || 'secret';
